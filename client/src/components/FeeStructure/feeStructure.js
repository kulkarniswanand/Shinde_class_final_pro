...
const FeesStructure = () => {
  ...
  return (
    <div className="p-6 bg-black min-h-screen text-white">
      ...
      {showUpdateForm && <UpdateFeesForm data={showUpdateForm} onClose={() => setShowUpdateForm(null)} fetchFees={fetchFeesData} branches={branches} />}
    </div>
  );
};
...
const UpdateFeesForm = ({ data, onClose, fetchFees, branches }) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-80 flex justify-center items-center">
      <div className="bg-gray-900 p-6 rounded-lg shadow-lg text-white">
        ...
        <form onSubmit={handleUpdate} className="space-y-3">
          ...
          <div className="flex justify-end gap-3">
            <button type="button" onClick={onClose} className="bg-gray-700 px-3 py-1 rounded">Cancel</button>
            <button type="submit" className="bg-blue-500 hover:bg-blue-600 px-3 py-1 rounded">Update</button>
          </div>
        </form>
      </div>
    </div>
