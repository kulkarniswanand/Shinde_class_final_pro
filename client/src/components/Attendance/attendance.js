...
const AttendanceDashboard = () => {
  ...
  const [rosterStats, setRosterStats] = useState({
    // Stats for the selected class & date
    totalStudents: 0,
    presentCount: 0,
    absentCount: 0,
    pendingCount: 0,
  ...
  useEffect(() => {
    const calculateChartData = () => {
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        const formattedDateLabel = `${date.toLocaleString("default", {
          month: "short",
        })} ${date.getDate()}`;
  ...
  useEffect(() => {
    ...
    const dateFilteredRecords = attendanceRecords.filter((record) => {
      try {
        ...
        const recordDateLocal = new Date(record.date).toLocaleDateString(
          "en-IN"
        ); // Use 'en-IN' for IST format
        const selectedDateLocal = new Date(selectedDate).toLocaleDateString(
          "en-IN"
        ); // Convert selected date to local format
  ...
  const getStudentAttendanceStatus = (
    ...
    const record = recordsToCheck.find((record) => {
      ...
      try {
        ...
        const [targetYear, targetMonth, targetDay] = dateToCheck
          .split("-")
          .map(Number);
    ...
    try {
      ...
      dateFilteredRecords.forEach((record) => {
        ...
        try {
          ...
          localDateLabel = recordDate.toLocaleDateString("en-US", {
            month: "short",
            day: "numeric",
          }); // Keep using local formatting for display label
        } catch (e) {
          return;
        } // Skip invalid dates
      ...
      const processedHistory = Object.values(summaryMap)
        .map((summary) => {
        ...
        .filter(
          (summary) => summary.presentCount > 0 || summary.absentCount > 0
        ); // Remove days with no attendance
  ...
  const handleMarkAttendance = async (student, status) => {
    ...
    try {
      ...
      const studentName =
        student.name || student.studentname || `Student ${studentId}`; // Handle different name fields
      ...
      if (!studentId || !studentName || !studentClass || !studentBranch) {
        console.error("Missing required student data for marking:", {
          studentId,
          studentName,
          studentClass,
          studentBranch,
        ...
        alert(
          "Student data (ID, Name, Class, Branch) is incomplete. Cannot mark attendance."
        );
  ...
  const viewRecordDetails = async (historySummaryRecord) => {
    ...
    try {
      ...
      if (
        historySummaryRecord.records &&
        historySummaryRecord.records.length > 0
      ) {
        const present = historySummaryRecord.records.filter(
          (r) => r.status === "present"
        );
        const absent = historySummaryRecord.records.filter(
          (r) => r.status === "absent"
        );
        ...
        const uniquePresent = Array.from(
          new Map(present.map((item) => [item.id, item])).values()
        );
        const uniqueAbsent = Array.from(
          new Map(absent.map((item) => [item.id, item])).values()
        );
        ...
        console.warn(
          "No detailed records found in history summary:",
        ...
        setRecordDetails({
          ...
          absentStudents: [],
          loading: false,
          error: "No detailed student records found for this entry.",
        });
      }
    } catch (error) {
  ...
  const getCalendarData = useMemo(() => {
    ...
    Object.keys(monthData).forEach((day) => {
      ...
      data.rate =
        total > 0 ? parseFloat(((data.present / total) * 100).toFixed(1)) : 0;
  ...
  const exportToPDF = (exportData, title = "Attendance Report") => {
    try {
      ...
      doc.setFontSize(10);
      doc.setTextColor(0, 0, 0);
      const presentStudents = Array.isArray(exportData.presentStudents)
        ? exportData.presentStudents
        : [];
      const absentStudents = Array.isArray(exportData.absentStudents)
        ? exportData.absentStudents
        : [];
      const totalStudents = presentStudents.length + absentStudents.length; // Use actual student counts if available
      const presentCount = presentStudents.length;
      const absentCount = absentStudents.length;
      let attendanceRate =
        exportData.attendanceRate ??
        (totalStudents > 0
          ? ((presentCount / totalStudents) * 100).toFixed(1)
          : "0");

      doc.text(`Total Students Listed: ${totalStudents}`, margin, y);
      y += 5;
      doc.text(`Present: ${presentCount}`, margin, y);
      y += 5;
      doc.text(`Absent: ${absentCount}`, margin, y);
      y += 5;
      doc.text(`Attendance Rate: ${attendanceRate}%`, margin, y);
      y += 10;
      ...
      const addStudentSection = (students, sectionTitle) => {
        ...
        if (students.length > 0) {
          students.forEach((student, index) => {
            ...
            doc.text(String(student.name || ""), margin + 30, y, {
              maxWidth: 65,
            }); // Add maxWidth
          ...
          doc.text(
            `No ${sectionTitle.toLowerCase()} found for this period.`,
            margin,
            y
          );
          y += 7;
      ...
      for (let i = 1; i <= pageCount; i++) {
        doc.setPage(i);
        doc.text(
          `Shinde Classes - Attendance Report - Page ${i} of ${pageCount}`,
          pageWidth / 2,
      ...
      // Save
      const safeClassName = (exportData.className || "all_classes")
        .replace(/[^a-z0-9]/gi, "_")
        .toLowerCase();
      const safeDate = (exportData.date || "report")
        .replace(/[^a-z0-9]/gi, "_")
        .toLowerCase();
  ...
  const prepareDetailedCSVData = (data) => {
    const csvData = [];
    const presentStudents = Array.isArray(data.presentStudents)
    ...
    if (presentStudents.length > 0) {
      presentStudents.forEach((s) =>
        csvData.push([s.id || "", s.name || "", s.class || "", s.branch || ""])
      );
    } else {
  ...
  const handleExportFromModalPDF = () => {
    if (selectedHistoryRecord && recordDetails) {
      const exportData = {
        date:
          selectedHistoryRecord.dateLabel ||
  ...
  const handleExportFromSearchPDF = async () => {
    ...
    try {
      ...
      filteredHistory.forEach((summaryRecord) => {
        if (summaryRecord.records) {
          summaryRecord.records.forEach((detailRecord) => {
            ...
            if (
              detailRecord.status === "present" &&
              !allPresentStudents.has(studentData.id)
            ) {
              allPresentStudents.set(studentData.id, studentData);
            } else if (
              detailRecord.status === "absent" &&
              !allAbsentStudents.has(studentData.id)
            ) {
      ...
      const totalPresent = filteredHistory.reduce(
        (sum, r) => sum + (r.presentCount || 0),
        0
      );
      const totalAbsent = filteredHistory.reduce(
  ...
  const handleExportFromSearchDetailedCSV = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `detailed_attendance_${safeClassName}_${historyDateRange.startDate}_to_${historyDateRange.endDate}.csv`
      );
  ...
  const isLoading =
    isClassesLoading || isStudentsLoading || isAttendanceLoading; // Combine initial loading states
  ...
  const totalRosterPages = Math.ceil(
    filteredRosterStudents.length / rosterRecordsPerPage
  );
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col">
          ...
          <nav className="flex space-x-5">
            {["dashboard", "roster", "history"].map((view) => {
              ...
              return (
                <button
                  key={view}
                  className={`px-4 py-2 flex items-center space-x-2 rounded-md transition-colors duration-150 ${
                    isActive
      ...
      <main className="container mx-auto px-8 py-8 flex-1">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
              ...
              <div className="bg-white rounded-lg shadow p-8">
                <h3 className="text-lg font-semibold text-gray-800 mb-6">
                  Overall Attendance Rate Trend (Last 7 Days)
                </h3>
                <div className="h-72">
                  {dashboardRateData.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart
                        data={dashboardRateData}
              ...
              <div className="bg-white rounded-lg shadow p-8">
                <h3 className="text-lg font-semibold text-gray-800 mb-6">
                  Overall Daily Breakdown (Last 7 Days)
                </h3>
                <div className="h-72">
        ...
        {activeView === "roster" && (
          <>
            <h2 className="text-2xl font-semibold text-gray-800 mb-8">
              Class Roster & Attendance Marking
            </h2>
              ...
              <div className="grid grid-cols-4 gap-4 mb-8">
                <div className="bg-white rounded-lg shadow p-4 text-center">
                  ...
                  <p className="text-2xl font-bold text-gray-800">
                    {rosterStats.totalStudents}
                  </p>
                </div>
                <div className="bg-white rounded-lg shadow p-4 text-center">
                  <p className="text-sm text-gray-600 mb-1">Present</p>
                  <p className="text-2xl font-bold text-green-600">
                    {rosterStats.presentCount}
                  </p>
                </div>
                <div className="bg-white rounded-lg shadow p-4 text-center">
                  <p className="text-sm text-gray-600 mb-1">Absent</p>
                  <p className="text-2xl font-bold text-red-600">
                    {rosterStats.absentCount}
                  </p>
                </div>
                <div className="bg-white rounded-lg shadow p-4 text-center">
                  <p className="text-sm text-gray-600 mb-1">Pending</p>
                  <p className="text-2xl font-bold text-amber-600">
                    {rosterStats.pendingCount}
                  </p>
                </div>
              </div>
            )}
            ...
            {/* Student Roster Cards */}
            {
              selectedClass === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {rosterPaginatedStudents.map((student) => {
                    ...
                    return (
                      <div
                        key={student.id}
                        className="bg-white rounded-lg shadow overflow-hidden transition duration-150 ease-in-out"
                      >
                          ...
                          <div className="flex items-center space-x-4">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-500">
                                ID: {studentId} | Roll: {studentRoll}
                              </p>
                              <p className="text-xs text-gray-500">
                                Class: {studentClass} | Branch: {studentBranch}
                              </p>
                            </div>
                          </div>
                          {/* Status Badge */}
                        ...
                        <div className="bg-gray-50 px-4 py-3 flex space-x-3">
                          <button
                            ...
                            disabled={
                              isMarkingAttendance || status === "present"
                            }
                            className={`flex-1 px-3 py-1.5 rounded text-xs font-medium border transition-colors duration-150 ${
                              isMarkingAttendance
                                ? "bg-gray-200 text-gray-500 cursor-not-allowed"
                                : status === "present"
                                ? "bg-green-500 text-white border-green-500 cursor-not-allowed opacity-80"
                          ...
                          <button
                            ...
                            disabled={
                              isMarkingAttendance || status === "absent"
                            }
                            className={`flex-1 px-3 py-1.5 rounded text-xs font-medium border transition-colors duration-150 ${
                              isMarkingAttendance
                                ? "bg-gray-200 text-gray-500 cursor-not-allowed"
                ...
                <div className="bg-yellow-50 border-l-4 border-yellow-400 p-5 text-center text-yellow-800 col-span-full">
                  No students found for the selected class or matching your
                  search criteria.
                </div>
            ...
            {selectedClass !== 0 && totalRosterPages > 1 && (
              <div className="flex justify-center items-center mt-8 space-x-2">
                <button
                  ...
                  onClick={() =>
                    setRosterCurrentPage((prev) => Math.max(prev - 1, 1))
                  }
                ...
                <button
                  className="bg-white text-blue-600 px-3 py-1 rounded border border-blue-300 hover:bg-blue-50 disabled:opacity-50 disabled:cursor-not-allowed"
                  onClick={() =>
                    setRosterCurrentPage((prev) =>
                      Math.min(prev + 1, totalRosterPages)
                    )
                  }
        ...
        {activeView === "history" && (
          <>
                            ...
                            >
                              ...
                              <td className="px-4 py-2 whitespace-nowrap text-right">
                                <button
                                  ...
                                  } // Disable if no details available
                                  title={
                                    !record.records ||
                                    record.records.length === 0
                    ...
                    <div className="flex justify-between items-center mt-4 text-sm">
                      <p className="text-gray-600">
                        ...
                        {Math.min(
                          ...
                          (historyPage - 1) * historyRecordsPerPage + 1
                        )}
                        to{" "}
                        {Math.min(
                          filteredHistory.length,
                ...
                <div className="mt-4">
                  ...
                  <div className="flex justify-between items-center mb-4 px-2">
                    <button
                      ...
                      className="text-sm p-1 rounded hover:bg-gray-100 text-gray-500"
                    >
                      &lt; Prev
                    </button>
                    <h4
                    ...
                    {(() => {
                      ...
                      for (let i = 0; i < firstDayOfMonth; i++) {
                        daysArray.push(
                          <div
                            key={`empty-${i}`}
                            className="border rounded bg-gray-50 h-20"
                          ></div>
                        );
                      }
                      ...
                      for (let day = 1; day <= daysInMonth; day++) {
                        ...
                        daysArray.push(
                          <div
                            ...
                            onClick={
                              canViewDetails
                                ? () => viewRecordDetails(detailsRecord)
                                : undefined
                            }
                            title={
                              canViewDetails
                                ? `Click to view details for ${detailsRecord.dateLabel}`
                                : isCurrentDay
                                ? "Today"
                                : ""
                            }
                          >
                            ...
                            {hasAttendance && dayData && (
                              <div className="mt-auto text-[10px] leading-tight space-y-0.5">
                                ...
                                <div
                                  className={`px-1 py-0.5 rounded text-center font-medium ${
                                    dayData.rate >= 90
                                      ? "bg-green-100 text-green-800"
                                      : dayData.rate >= 75
                                      ? "bg-yellow-100 text-yellow-800"
                  ...
                  <div className="mt-4 flex flex-wrap justify-center gap-x-4 gap-y-1 text-xs text-gray-500">
                    <div className="flex items-center">
                      <div className="h-2 w-2 rounded-full bg-green-100 mr-1 border border-green-300"></div>
                      ≥ 90%
                    </div>
                    <div className="flex items-center">
                      <div className="h-2 w-2 rounded-full bg-yellow-100 mr-1 border border-yellow-300"></div>
                      75-90%
                    </div>
                    <div className="flex items-center">
                      <div className="h-2 w-2 rounded-full bg-red-100 mr-1 border border-red-300"></div>
                      &lt; 75%
                    </div>
                    <div className="flex items-center">
                      <div className="h-2 w-2 rounded-full bg-blue-100 mr-1 border border-blue-300"></div>
                      Today
                    </div>
                    <div className="flex items-center">
                      <div className="h-2 w-2 rounded-full bg-white mr-1 border border-gray-300"></div>
                      No Data / Filtered Out
                    </div>
                  </div>
                </div>
              )}
            ...
            {showRecordDetails && selectedHistoryRecord && (
              ...
              >
                <div
                  className="bg-white rounded-lg shadow-xl p-6 max-w-2xl w-full mx-auto relative animate-fade-in-up"
                  onClick={(e) => e.stopPropagation()}
                >
                  {/* Modal Header */}
                  <div className="flex justify-between items-start mb-4 pb-3 border-b">
                    <h3 className="text-lg font-semibold text-gray-900">
                      Attendance Details: {selectedHistoryRecord.className} -{" "}
                      {selectedHistoryRecord.dateLabel}
                    </h3>
                    <button
                      ...
                      <div className="grid grid-cols-3 gap-4 mb-6 text-center">
                        <div className="bg-green-50 p-3 rounded">
                          ...
                          <p className="text-xl font-bold text-green-700">
                            {recordDetails.presentStudents.length}
                          </p>
                        </div>
                        <div className="bg-red-50 p-3 rounded">
                          ...
                          <p className="text-xl font-bold text-red-700">
                            {recordDetails.absentStudents.length}
                          </p>
                        </div>
                        <div className="bg-blue-50 p-3 rounded">
                          ...
                          <p className="text-xl font-bold text-blue-700">
                            {selectedHistoryRecord.attendanceRate}%
                          </p>
                        </div>
                      </div>
                  ...
                  <div className="flex justify-end space-x-3 mt-6 pt-4 border-t">
                    <CSVLink
                      ...
                      className={`inline-block bg-green-500 text-white px-4 py-1.5 rounded text-sm hover:bg-green-600 ${
                        recordDetails.loading || recordDetails.error
                          ? "opacity-50 pointer-events-none"
                          : ""
                      }`}
      ...
      <footer className="w-full bg-gray-800 text-white py-4 text-center mt-auto">
        <p className="text-sm">
          © {new Date().getFullYear()} Shinde Classes - Attendance Management
        </p>
      </footer>
    </div>
  );
