...
const AttendanceDashboard = () => {
    ...
    useEffect(() => {
        ...
        const dateFilteredRecords = attendanceRecords.filter(record => {
            try {
                return new Date(record.date).toISOString().split('T')[0] === selectedDate; // Ensure this matches the local date format
    ...
    }, [filteredRosterStudents, attendanceRecords, selectedClass, selectedDate]); // Ensure attendanceRecords is included in dependencies
    ...
    const getStudentAttendanceStatus = (studentId, recordsToCheck, dateToCheck) => {
        ...
        const targetDateLocal = new Date(dateToCheck).toLocaleDateString('en-IN'); // Convert to local date string
        const record = recordsToCheck.find(record => {
            const matchesStudent = String(record.id) === String(studentId);
            if (!matchesStudent) return false;

            const recordDateObj = new Date(record.date);
            const recordDateLocal = recordDateObj.toLocaleDateString('en-IN'); // Convert record date to local format

            return recordDateLocal === targetDateLocal; // Compare local date strings
        });
